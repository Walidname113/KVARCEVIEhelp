...
rm -rf install.sh update.sh code.py amain.py bootlibralies.sh configure.sh words.json > /dev/null 2>&1

for file in command_prefix.txt chat_ids.txt interval.txt sessioncash.txt send_status.txt ocrestrinatedub.session; do
    if [ -f "$file" ]; then
        mv "$file" ~ > /dev/null 2>&1 || { echo -e "$REDОшибка перемещения $file!$NC"; }
    fi
done
...
for file in command_prefix.txt chat_ids.txt interval.txt sessioncash.txt send_status.txt ocrestrinatedub.session; do
    if [ -f ~/"$file" ]; then
        mv ~/"$file" KRAVIENCEhelp > /dev/null 2>&1 || { echo -e "$REDОшибка перемещения $file обратно!$NC"; }
    fi
done
